...
resVap=fit_sin(meanVapArr, meanPhiArr)
VapfitArr = resVap["fitfunc"](fineVapArr)

fineVinArr= np.linspace(min(meanVinArr), max(meanVinArr), 100)
resVin=fit_sin(meanVinArr, meanPhiArr)
VinfitArr = resVap["fitfunc"](fineVinArr)

fineTArr= np.linspace(min(meanTArr), max(meanTArr), 100)
resT=fit_sin(meanTArr, meanPhiArr)
TfitArr = resT["fitfunc"](fineTArr)

maxVapFit = max(meanVapArr)
minVapFit=min(meanVapArr)
visib = (maxVapFit-minVapFit)/(maxVapFit+minVapFit)
...
#Phi vs Vap
phimaxfit = max(VapfitArr)
phiminfit = min(VapfitArr)

axs[0].errorbar(meanVapArr, meanPhiArr, yerr=np.std(meanPhiArr), linestyle = 'none', marker = '.', markersize = 4)
axs[0].plot(fineVapArr,VapfitArr, '-r')
axs[0].set_ylabel(r"Mean $\phi (\degree)$")
axs[0].set_xlabel(r"Mean $V_{ap}$ (V)")
axs[0].set_title("Phase vs. Voltage Sent to Power Supply "+ r"($V_{ap}$)")
axs[0].grid()

#Phi vs Vin
axs[1].errorbar(meanVinArr, meanPhiArr, yerr=np.std(meanPhiArr), linestyle = 'none', marker = '.', markersize = 4)
axs[1].plot(fineVinArr,VinfitArr, '-r')
axs[1].set_ylabel(r"Mean $\phi$ ($\degree$)")
axs[1].set_xlabel(r"Mean $V_{in}$ (V)")
axs[1].set_title("Phase vs. Actual Voltage of Power Supply "+ r"($V_{ap}$)")
axs[1].grid()

#Phi vs T
axs[2].errorbar(meanTArr, meanPhiArr, yerr=np.std(meanPhiArr), linestyle = 'none', marker = '.', markersize = 4)
axs[2].plot(fineTArr,TfitArr, '-r')
axs[2].set_ylabel(r"Mean $\phi$ ($\degree$)")
...
fig.suptitle("Fit Visibility: {}".format(visib))
plt.show()
